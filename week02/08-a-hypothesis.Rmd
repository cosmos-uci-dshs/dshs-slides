...
```{r, echo=FALSE,out.width='40%',out.height='60%',fig.align='center'}
knitr::include_graphics('img/pValLower.png')
```
...
- A common \textbf{mistake} is to regard the $p$-value as the probability of null given the observed test statistic:
$P(H_{0} | \bar{X} = \bar{x})$. 
...
- For these hypotheses, $p_{\mathrm{obs}} = P(Z \le z)$ and $p_{\mathrm{obs}} = P(Z \ge z)$ respectively.
...
- In this case, $p_{\mathrm{obs}} = 2 \times P(Z \geq |z|)$.
...
- This means that
$$\begin{equation*}
Z  =  \frac{\bar{X} - \mu_{0}}{\sqrt{ \mu_{0}(1-\mu_{0})/n}} \sim N(0, 1).
\end{equation*}$$
- As a result, we obtain the $z$-score as follows, where $p$ is the sample proportion (mean)
$$\begin{equation*}
